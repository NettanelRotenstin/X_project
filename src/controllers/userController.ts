...
router.get("/search", async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await UserService.getOneUser(req.query.userName as string)
    res.json({
      err: false,
      message: "I was too lazy to change the default message",
      data: result,
...
router.get("/profile", async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await UserService.getOneUser(req.query.id as string)
...
router.get("/followers", async (req: Request, res: Response): Promise<void> => {
  try {
    const result :User | undefined = await UserService.getOneUser(req.query.id as string)
    res.json({
      err: false,
      message: "I was too lazy to change the default message",
